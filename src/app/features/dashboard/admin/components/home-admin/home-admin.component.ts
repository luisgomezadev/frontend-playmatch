    ...
    this.authService.currentUser$.subscribe(user => {
      if (user) {
        ...
        if (this.user.field?.id) {
          ...
          this.reservationService.getCountActiveByField(this.user.field.id).subscribe({
            next: (value) => {
              this.reservationsActive = value;
            }
          });
          this.reservationService.getCountFinishedByField(this.user.field.id).subscribe({
            next: (value) => {
